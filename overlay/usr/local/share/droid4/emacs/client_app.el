...
(defun send-sms (button)
 (save-excursion
  ...
  (let ((msg (filter-buffer-substring 
    ...
    (shell-command (format "/usr/local/bin/droid4-modem send -p '%s' %s" droid-phone-number msg)))))
					;    (message (format "sms -p '%s' %s" droid-phone-number msg)))))
(defun call-action (button)
 (shell-command (format "/usr/local/bin/droid4-modem call '%s'" droid-phone-number))) 
...
(defun options-page (button)
 (save-excursion
  (let ((option-list '(("send sms" send-sms-page)
    ("call" call-action)
